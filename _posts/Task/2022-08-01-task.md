...
3. ~~Player Status UI 제작 및 연결 - Player lives, stamina~~
4. DisPose 조사(자원 사용후 반납): Delegate는 반납개념없음, (잘모르겟음) https://stackoverflow.com/questions/12566584/what-to-do-with-delegate-event-references-in-a-class-that-implements-idisposab
5. ~~Stamina 바 이벤트 핸들러를 사용해야만 할까~~(일단 지금은 어쩔수 없음)
6.  https://forum.unity.com/threads/delete-all-components-of-gameobject-except-two.141009/ Player 스크립트는 그대로 두고 나머지 삭제
7.  stamina bar: Cant Use stamina 상태에 대한 애니메이션(좌우로 흔들리는 거), Gauge 차는거 애니메이션 구현
