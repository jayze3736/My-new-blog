...
Unity 엔진은 C++로 만들어져있다. 그러나 우리가 C#의 기능을 사용하기위해서는 기존의 C++ 데이터를 C# 환경(.NET)에서 사용하기위해서 만들어진 래퍼 클래스가 바로 UnityEngine.Object 클래스이다.   
...
## - Destroy 함수
Object로 래핑되어있는 객체는 두가지 객체를 가진다. 하나는 원래 데이터에 해당하는 C++ 객체(또는 native 객체) 그리고 다른 하나는 래핑되면서 만들어진 C#(UnityEngine.Object) 객체이다.  
